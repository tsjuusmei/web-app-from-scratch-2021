...
var router = new _navigo.default(null, false);
var photosUrl = 'https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?earth_date=2021-1-31&api_key=i8b5WtpKTdfadlaSh7So7ZZmn7T7q2UfDSaS3ABs';
var tempsUrl = 'https://wafs-be.herokuapp.com/temps';
...
function averageTemperature(data) {
  ...
  for (var i = 0; i < averageTemps.length; i++) {
    temps += averageTemps[i];
  }

  var averageTemp = temps / averageTemps.length;
  appendTemp(averageTemp.toFixed(2));
}

var photoData = fetch(photosUrl).then(function (response) {
  return response.json();
}).then(function (data) {
  console.log(data);
  randomPhoto(data.photos);
});
var tempData = fetch(tempsUrl).then(function (res) {
  return res.json();
}).then(function (d) {
  return averageTemperature(d);
});
},{"navigo":"../node_modules/navigo/lib/navigo.min.js"}],"../../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
